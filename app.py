...
def reply_handler(bot, update):
    ...
    # 新增 攝像機 功能
        ...
        if text == "Y" or text == "y":
            ...
            cameraCreate = False
            createList = []
        elif len(createList) < 4 and Y_check == True:
            respText = ""
            ...
            if len(createList) == 4:
                ...
                dbCameraCreate.update_one({"feature":"datapost"}, {"$set":{"status":"1", "name":createList[0], "location":createList[1], "ip":createList[2], "port":createList[3], "chat_id":update.message.chat_id}}, upsert=True)
                print("Request send")
                Y_check = False
                cameraCreate = False
                createList = []
...
def camera_select(bot, update):
    ...
    if mode == 'photo' or mode == 'video':
        ...
        bot.send_message(chat_id=update.callback_query.message.chat_id, text=respText, reply_markup = InlineKeyboardMarkup([
            [InlineKeyboardButton('攝影機' + str(i), callback_data = "camera_select:" + str(i) + ":" + mode)] for i in range(1, count_list+1)
        ]), parse_mode="Markdown")
    elif mode == 'create':
