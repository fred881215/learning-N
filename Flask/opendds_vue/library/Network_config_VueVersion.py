...
class File_search():
    def __init__(self):
        if (os.path.isdir("/home/nick/pi/ini")): pass
        else: os.mkdir("/home/nick/pi/ini") 

    def ini_list(self):
        self.ini_table = os.listdir('/home/nick/pi/ini/')
...
class Net_config():
    def __init__(self):
        try:
            ...
            os.system('sudo cp ./library/interfaces_copy.bak /etc/network/interfaces')
        try:
    ...
    def eth0_status(self):
        command = 'ip address show dev wlp3s0 | grep "wlp3s0" | awk "END {print}"'
        ...
        command = 'ip r show dev wlp3s0 | grep default'
        result = subprocess.Popen(command, shell=True, stdout=subprocess.PIPE, stderr=subprocess.STDOUT)
        try:
    ...
    def eth0_dhcp(self):
        os.system("sudo sed -i '3c iface wlp3s0 inet dhcp' /etc/network/interfaces")
        ...
        os.system('sudo ifdown wlp3s0')
        os.system('sudo ifup --ignore-errors wlp3s0')
        os.system('sudo supervisorctl restart all')
        os.system('sudo cp /etc/network/interfaces ./library/interfaces_copy.bak')
    ...
    def eth0_static(self, ip, netmask, gateway):
        os.system("sudo sed -i '3c iface wlp3s0 inet static' /etc/network/interfaces")
